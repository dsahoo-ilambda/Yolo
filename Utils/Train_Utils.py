...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, random=False):
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            # box contains (x1,y1,x2,y2,class_label)
            image, box = get_random_data(annotation_lines[i], input_shape, random=random)
            image_data.append(image)
            box_data.append(box)
            i = (i + 1) % n
        image_data = np.array(image_data)
        # Collects boxes for a whole batch
        box_data = np.array(box_data)
        # box needs to be converted to model output format
        # y_true shape = (batch_size, 14, 14, 3, 55) for first detection layer
        # y_true is an array of true_outputs of 3 detection layers
        y_true = preprocess_true_boxes(box_data, input_shape, anchors, num_classes)
        yield [image_data, *y_true], np.zeros(batch_size)


def data_generator_wrapper(annotation_lines, batch_size, input_shape, anchors, num_classes, random=False):
    n = len(annotation_lines)
    if n == 0 or batch_size <= 0: return None
    return data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes, random=random)
